...
public class Prog {
    ...
    private Deque<Instruction> nextThreeInstructions = new ArrayDeque<Instruction>();
    static {
    ...
    public int branchToInstruction(Integer amountOfInstructionsToJumpFrom){
        Instruction empty = PipelineRegisters.getEmpty();
        Instruction first = (PC <= MAX_COMMAND) ? instructions.get(PC) : empty;
        Instruction second = (PC + 1 <= MAX_COMMAND) ? instructions.get(PC + 1) : empty;
        Instruction third = (PC + 2 <= MAX_COMMAND) ? instructions.get(PC + 2) : empty;

        nextThreeInstructions.add(first);
        nextThreeInstructions.add(second);
        nextThreeInstructions.addLast(third);
    ...
    public Instruction getInstructionAfterBranch(){
        return nextThreeInstructions.pollFirst();
    }
